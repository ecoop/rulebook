...
"""End-to-end per-user domain access (#112): the allowlist is wired to routes.

Local backend, so `/meta` falls back to DEFAULT_DOMAINS and the allowlist comes
from `initial_allowed_domains` + the configured default. Proves the filtering /
enforcement dependencies are actually on the endpoints — the unit tests in
test_allowed_domains.py can't see that.
"""
...
def test_meta_and_me_filter_to_allowlist(client):
    ...
    _as(client, "tok_ult")
    assert client.get("/meta").json()["domains"] == ["ultimate"]
    assert client.get("/me").json()["allowed_domains"] == ["ultimate"]


def test_meta_uses_default_for_ungranted_user(client):
    # No explicit grant → the concrete default (both current domains), NOT all.
    # /meta order follows the index; compare as a set. A domain added later
    # (badminton, curling) is not in the default, so it stays absent here.
    _as(client, "tok_nov")
    assert set(client.get("/meta").json()["domains"]) == {"ultimate", "goaltimate"}
...
def test_allowlist_read_is_local_but_writes_need_gcs(client):
    ...
    assert (
        client.post(
            "/advanced/allowed-domains", json={"token": "tok_nov", "domains": ["ultimate"]}
        ).status_code
        == 400
    )
